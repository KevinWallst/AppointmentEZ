...
// Check if a time slot is available (excluding the current booking)
const isTimeSlotAvailable = (bookings: Booking[], appointmentTime: string, currentBookingId: string): boolean => {
...
const sendConfirmationEmail = async (booking: Booking) => {
  ...
  try {
    console.log('Attempting to send update confirmation email with the following configuration:');
...
export async function PUT(request: Request) {
  try {
    ...
    if (!data.id || !data.appointmentTime || !data.name || !data.email || !data.wechatId || !data.topic) {
    ...
    console.log('Reading existing bookings for update operation');
    let bookings = readBookings();
    console.log(`Found ${bookings.length} existing bookings`);

    // Find the booking to update
    const bookingIndex = bookings.findIndex(booking => booking.id === data.id);

    if (bookingIndex === -1) {
      console.error(`Booking with ID ${data.id} not found`);
      return NextResponse.json(
        { error: 'Booking not found' },
        { status: 404 }
      );
    }
    console.log(`Found booking to update at index ${bookingIndex}`);

    // Check if the time slot is available (if changing the time)
    if (data.appointmentTime !== bookings[bookingIndex].appointmentTime) {
      console.log('Appointment time is being changed, checking availability');
      console.log('Original time:', bookings[bookingIndex].appointmentTime);
      console.log('New time:', data.appointmentTime);

      if (!isTimeSlotAvailable(bookings, data.appointmentTime, data.id)) {
        console.error('ERROR: Requested time slot is already booked');
        console.error('Appointment time:', data.appointmentTime);
        console.error('Booking ID:', data.id);
        console.error('Conflicting bookings:', bookings.filter(booking => {
          if (booking.id === data.id) return false; // Skip the current booking
      ...
      // Find the booking again in the latest data
      const latestBookingIndex = latestBookings.findIndex(booking => booking.id === data.id);

      if (latestBookingIndex === -1) {
        console.error(`Booking with ID ${data.id} was deleted while processing update`);
        return NextResponse.json(
          { error: 'Booking not found', details: 'The booking was deleted while processing your update' },
          { status: 404 }
        );
      }

      // Check again if the time slot is still available
      if (!isTimeSlotAvailable(latestBookings, data.appointmentTime, data.id)) {
      ...
      console.log('Appointment time is not being changed');
    }
    ...
    const updatedBooking: Booking = {
      ...
      topic: data.topic,
      language: data.language || bookings[bookingIndex].language || 'zh' // Preserve existing language or default to Chinese
    };
    ...
    // Send confirmation email for the updated appointment
    let emailSent = false;
    try {
      ...
      if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
        ...
        const emailResult = await sendConfirmationEmail(updatedBooking);
        if (emailResult) {
          console.log('Update confirmation email sent successfully');
          emailSent = true;
        } else {
    ...
    return NextResponse.json(
      { success: true, booking: updatedBooking, emailSent: emailSent },
      { status: 200 }
    );
  } catch (error) {
